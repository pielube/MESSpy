    ...
    def __init__(self,system,general,location_name,path,check,file_structure,file_general):
        ...
        for carrier in self.power_balance: 
            ...
            if f"{carrier} grid" in self.system: 
                self.power_balance[carrier]['grid'] = np.zeros(self.timestep_number) # array energy carrier bought from the grid (-) or feed into the grid (+)
            ...
            if f"{carrier} demand" in self.system: # read and check demand series
                ...
                if carrier in ['hydrogen','HP hydrogen','process steam'] and self.system[carrier+' demand']['serie'] != False:
                    ...
                    if self.system[carrier+' demand']['strategy'] == 'supply-led' and self.system[carrier+' demand']['serie'] == False:
                        self.power_balance[carrier]['demand'] =  np.zeros(self.timestep_number)
                ...
                if len(self.power_balance[carrier]['demand']) == self.timestep_number:
                    pass 
                elif len(self.power_balance[carrier]['demand'])<self.timestep_number:
                    if len(self.power_balance[carrier]['demand'])/self.timestep < 8760:
                    ...
                    elif self.timestep_number%len(self.power_balance[carrier]['demand']) == 0:
                        print(f"Warning! You are asking for a simulation of {general['simulation length']} years, but the {carrier} demand series you input for {self.name} is only {self.timestep_number/len(self.power_balance[carrier]['demand'])}  years long. It will automatically be repeated.")
                        self.power_balance[carrier]['demand'] = - np.tile(self.power_balance[carrier]['demand'],self.timestep_number/len(self.power_balance[carrier]['demand']))   
                else:
                    raise ValueError(f"Warning! Checks the length of the {carrier} demand series you input for {self.name}. In general.json it checks the timestep and the simulation length")

        if 'chp_gt' in self.system:
            self.technologies['chp_gt'] = chp_gt(system['chp_gt'],self.timestep_number) # chp_gt object created and added to 'technologies' dictionary
            self.power_balance['process steam']['chp_gt'] = np.zeros(self.timestep_number) # array chp_gt process steam balance 
            self.power_balance['electricity']['chp_gt'] = np.zeros(self.timestep_number) # array chp_gt electricity balance
            self.power_balance['hydrogen']['chp_gt'] = np.zeros(self.timestep_number) # array chp_gt process hydrogen balance 
        
        if 'chp' in self.system:
            self.technologies['chp'] = Chp(system['chp'],self.timestep_number) # chp object created and added to 'technologies' dictionary
            self.power_balance[self.technologies['chp'].th_out]['chp'] = np.zeros(self.timestep_number) # array chp thermal output balance (process steam/hot water)
            self.power_balance['electricity']['chp'] = np.zeros(self.timestep_number) # array chp electricity balance
            self.power_balance[self.technologies['chp'].fuel]['chp'] = np.zeros(self.timestep_number) # array chp fuel consumption balance
            self.power_balance['process heat']['chp'] = np.zeros(self.timestep_number) # array chp process heat balance
            self.power_balance['process hot water']['chp'] = np.zeros(self.timestep_number) # array chp process hot water balance
            self.power_balance['process cold water']['chp'] = np.zeros(self.timestep_number) # array chp process cold water balance
       
        if 'absorber' in self.system:
            self.technologies['absorber'] = Absorber(system['absorber'],self.timestep_number) # absorber object created and added to 'technologies' dictionary
            self.power_balance['process heat']['absorber'] = np.zeros(self.timestep_number) # array absorber process steam balance 
            self.power_balance['process hot water']['absorber'] = np.zeros(self.timestep_number) # array absorber process steam balance 
            self.power_balance['process cold water']['absorber'] = np.zeros(self.timestep_number) # array absorber process steam balance 
            
        if 'heatpump' in self.system:
            self.technologies['heatpump'] = heatpump(system['heatpump'],self.timestep_number) # heatpump object created and add to 'technologies' dictionary
            self.power_balance['electricity']['heatpump'] = np.zeros(self.timestep_number) # array heatpump electricity balance
            self.power_balance['heating water']['heatpump'] = np.zeros(self.timestep_number) # array heatpump heat balance
            self.power_balance['heating water']['inertial TES'] = np.zeros(self.timestep_number) # array inertial tank heat balance
        ...
        if 'boiler_el' in self.system:
            ...
            self.power_balance['electricity']['boiler_el'] = np.zeros(self.timestep_number) # array boiler_el electricity balance
            self.power_balance['heating water']['boiler_el'] = np.zeros(self.timestep_number) # array boiler_el heat balance
        ...
        if 'boiler_ng' in self.system:
            ...
            self.power_balance['gas']['boiler_ng'] = np.zeros(self.timestep_number) # array boiler_ng gas balance
            self.power_balance['heating water']['boiler_ng'] = np.zeros(self.timestep_number) # array boiler_ng heat balance 

        if 'PV' in self.system:
            self.technologies['PV'] = PV(self.system['PV'],general,self.timestep_number,self.timestep,self.name,path,check,file_structure,file_general) # PV object created and add to 'technologies' dictionary
            self.power_balance['electricity']['PV'] = np.zeros(self.timestep_number) # array PV electricity balance
           
        if 'inverter' in self.system:
            self.technologies['inverter'] = inverter(self.system['inverter'],self.timestep_number) # inverter object created and add to 'technologies' dictionary
            self.power_balance['electricity']['inverter'] = np.zeros(self.timestep_number)        # array inverter electricity balance
            
        if 'wind' in self.system:
            self.technologies['wind'] = wind(self.system['wind'],self.timestep_number,self.timestep,path)    # wind object created and add to 'technologies' dictionary
            self.power_balance['electricity']['wind'] = np.zeros(self.timestep_number)        # array wind electricity balance 
           
        if 'battery' in self.system:
            self.technologies['battery'] = battery(self.system['battery'],self.timestep_number)    # battery object created and to 'technologies' dictionary
            self.power_balance['electricity']['battery'] = np.zeros(self.timestep_number)         # array battery electricity balance
                           
        if 'electrolyzer' in self.system:
            self.technologies['electrolyzer'] = electrolyzer(self.system['electrolyzer'],self.timestep_number) # electrolyzer object created and to 'technologies' dictionary
            self.power_balance['electricity']['electrolyzer']              = np.zeros(self.timestep_number) # array electrolyzer electricity balance
            self.power_balance['oxygen']['electrolyzer']                   = np.zeros(self.timestep_number) # array electrolyzer oxygen balance
            self.power_balance['water']['electrolyzer']                    = np.zeros(self.timestep_number) # array electrolyzer water balance
            self.power_balance['hydrogen']['electrolyzer']                 = np.zeros(self.timestep_number) # array electrolyzer hydrogen balance
        ...
        if 'fuel cell' in self.system:
            self.technologies['fuel cell'] = fuel_cell(self.system['fuel cell'],self.timestep_number,self.timestep) # Fuel cell object created and to 'technologies' dictionary
            self.power_balance['electricity']['fuel cell']     = np.zeros(self.timestep_number)     # array fuel cell electricity balance
            self.power_balance['hydrogen']['fuel cell']        = np.zeros(self.timestep_number)     # array fuel cell hydrogen balance
            self.power_balance['heating water']['fuel cell']   = np.zeros(self.timestep_number)     # array fuel cell heat balance used
            
        if 'hydrogen compressor' in self.system:
            self.technologies['hydrogen compressor'] = hydrogen_compressor(self.system['hydrogen compressor'],self.timestep_number) # MHHC compressor object created and to 'technologies' dictionary
            self.power_balance['hydrogen']['hydrogen compressor']  = np.zeros(self.timestep_number)     # array hydrogen compressor hydrogen compressed
            self.power_balance['gas']['hydrogen compressor']       = np.zeros(self.timestep_number)     # array hydrogen compressor heating water balanced used
        
        if 'H tank' and 'HPH tank' in self.system: 
            self.technologies['H tank'] = H_tank(self.system['H tank'],self.timestep_number) # H tank object created and to 'technologies' dictionary
            self.power_balance['hydrogen']['H tank'] = np.zeros(self.timestep_number)         # array H tank hydrogen balance
            self.technologies['HPH tank'] = HPH_tank(self.system['HPH tank'],self.timestep_number)   # HPH tank object created and to 'technologies' dictionary
            self.power_balance['HP hydrogen']['HPH tank'] = np.zeros(self.timestep_number)        # array HPH tank hydrogen balance
        ...
        if 'O2 tank' in self.system: 
            self.technologies['O2 tank'] = O2_tank(self.system['O2 tank'],self.timestep_number) # LPH tank object created and to 'technologies' dictionary
            self.power_balance['oxygen']['O2 tank'] = np.zeros(self.timestep_number)         # array LPH tank hydrogen balance
        ...
        if 'mechanical compressor' in self.system:
            ...
            self.technologies['mechanical compressor'] = Compressor(self.system['mechanical compressor'],self.timestep_number,maxflowrate = maxflowrate) # H tank object created and to 'technologies' dictionary
            self.power_balance['electricity']['mechanical compressor']    = np.zeros(self.timestep_number) # array H tank hydrogen balance
            self.power_balance['hydrogen']['mechanical compressor']       = np.zeros(self.timestep_number) # array of hydrogen flow entering the mechanical compressor from LPH tank
            self.power_balance['HP hydrogen']['mechanical compressor']    = np.zeros(self.timestep_number) # array of compressed hydrogen flow sent toward HPH tank
            self.power_balance['cooling water']['mechanical compressor']  = np.zeros(self.timestep_number) # array of water flow to be fed to the refrigeration system 
        ...
        if 'boiler_h2' in self.system:
            ...
            self.power_balance['hydrogen']['boiler_h2'] = np.zeros(self.timestep_number)          # array boiler_h2 gas balance
            self.power_balance['heating water']['boiler_h2'] = np.zeros(self.timestep_number)     # array boiler_h2 heat balance 

        self.power_balance['electricity']['collective self consumption']   = np.zeros(self.timestep_number) # array contribution to collective-self-consumption as producer (-) or as consumer (+)
        self.power_balance['heating water']['collective self consumption'] = np.zeros(self.timestep_number) # array contribution to collective-self-consumption as producer (-) or as consumer (+)---heat----mio!!!
        self.power_balance['process steam']['collective self consumption'] = np.zeros(self.timestep_number) # array contribution to collective-self-consumption as producer (-) or as consumer (+)---heat----mio!!!
        ...
        for tech_name in self.system: # (which is ordered py priority)
            ...
            if tech_name == 'battery':
                if self.technologies['battery'].collective == 0: 
                    self.power_balance['electricity']['battery'][step] = self.technologies['battery'].use(step,self.timestep,pb['electricity']) # electricity absorbed(-) or supplied(+) by battery
            ...
            if tech_name == 'electrolyzer':
                ...
                if self.technologies['electrolyzer'].strategy == 'hydrogen-first' and self.technologies['electrolyzer'].only_renewables == True: # electrolyzer activated when renewable energy is available
                    if pb['electricity'] > 0: # electrolyzer activated only when renewable energy is available
                        ...
                        if producible_hyd > 0:
                            ...
                            self.power_balance['water']['electrolyzer'][step]        = self.technologies['electrolyzer'].use(step,self.timestep,storable_hydrogen=producible_hyd,e=pb['electricity'])      # [:2] # hydrogen supplied by electrolyzer(+) # electricity absorbed by the electorlyzer(-) 
                ...
                if step == (self.timestep_number - 1) and ('hydrogen demand' in self.system or 'HP hydrogen demand' in self.system):
                    if self.system[self.hydrogen_demand+' demand']['strategy'] == 'supply-led':  # activates only at the final step of simulation
                        self.constant_flow = sum(self.power_balance['hydrogen']['electrolyzer'])/self.timestep_number # [kg/s] constant hydrogen output based on the total production
            ...
            #     elif self.system['hydrogen demand']['strategy'] == 'supply-led' and h == (self.timestep_number - 1):
            #         prod = self.power_balance['hydrogen']['electrolyzer']
            #         for h in range(self.timestep_number):
            ...
            if tech_name == 'H tank':
                if 'HPH tank' not in self.system and ('hydrogen demand' in self.system or 'HP hydrogen demand' in self.system):
                    if self.system[self.hydrogen_demand+' demand']['strategy'] == 'demand-led':
                        ...
                        pb['hydrogen'] += self.power_balance['hydrogen']['H tank'][step]
                    elif self.system[self.hydrogen_demand+' demand']['strategy'] == 'supply-led' and step == (self.timestep_number - 1):
                        prod = self.power_balance['hydrogen']['electrolyzer']
                        for step in range(self.timestep_number):
            ...
            if tech_name == 'O2 tank':
                ...
                elif self.system['hydrogen demand']['strategy'] == 'supply-led' and step == (self.timestep_number - 1):
